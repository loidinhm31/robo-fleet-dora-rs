                            ...
                            if array.len() > 0 {
                                ...
                                match serde_json::from_slice::<RoverCommandWithMetadata>(bytes) {
                                    Ok(cmd_with_metadata) => {
                                        ...
                                        info!(
                                            "Received {} rover command (priority {:?})",
                                            source, cmd_with_metadata.metadata.priority
                                        );
                                        rover_controller.manual_command = Some(cmd_with_metadata);

                                        // Process arbitrated command
                                        if let Err(e) = rover_controller
...
impl RoverController {
        ...
        match (&self.manual_command, &self.servo_command) {
            (Some(manual), Some(servo)) => {
                ...
                if servo.metadata.priority >= manual.metadata.priority {
                    info!(
                        "Using servo command (priority {:?} >= {:?})",
                        servo.metadata.priority, manual.metadata.priority
                    );
                    Some(servo)
                } else {
                    info!(
                        "Manual override - using manual command (priority {:?} > {:?})",
    ...
    fn process_command(&mut self, command: RoverCommand) -> Result<()> {
        match &command {
            RoverCommand::Velocity {
                omega_z, v_x, v_y, ..
            } => {
                debug!(
                    "Processing velocity command: v_x={:.2} m/s, v_y={:.2} m/s, ω_z={:.2} rad/s",
                    v_x, v_y, omega_z
                );
                ...
                if linear_speed > self.safety_limits.max_linear_velocity {
                    ...
                    twist.v_y *= scale;
                    info!(
                        "SAFETY: Limited linear velocity from {:.2} to {:.2} m/s",
                ...
                if twist.omega_z.abs() > self.safety_limits.max_angular_velocity {
                    twist.omega_z =
                        twist.omega_z.signum() * self.safety_limits.max_angular_velocity;
                    info!(
                        "SAFETY: Limited angular velocity to {:.2} rad/s",
                ...
                debug!(
                    "Computed wheel speeds: [{:.2}, {:.2}, {:.2}] rad/s",
                ...
                for (i, &speed) in wheel_speeds.iter().enumerate() {
                    if speed.abs() > self.safety_limits.max_wheel_speed {
                        warn!(
                            "SAFETY: Wheel {} speed {:.2} exceeds limit {:.2} rad/s",
                            i,
                            speed.abs(),
                            self.safety_limits.max_wheel_speed
                        );
                    }
                }

                // Integrate wheel speeds to get position changes
                let delta_positions = self
                    .kinematics
                    .wheel_speeds_to_positions(&wheel_speeds, self.control_rate_dt);
                ...
                debug!(
                    "Updated wheel positions: [{:.3}, {:.3}, {:.3}] rad",
            ...
            RoverCommand::JointPositions {
                ...
                info!(
                    "Processing direct joint position command: [{:.3}, {:.3}, {:.3}] rad",
            ...
            RoverCommand::Legacy {
                ...
                info!(
                    "Processing legacy command: throttle={:.2}, brake={:.2}, steer={:.2}°",
