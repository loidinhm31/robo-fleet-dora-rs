...
if [ ! -f "/models/ggml/ggml-tiny.bin" ]; then
    echo "ERROR: Whisper model not found at /models/ggml/ggml-tiny.bin"
    echo ""
    echo "Please ensure the models directory is mounted correctly:"
    echo "  - Host: ./models/.cache/ggml/ggml-tiny.bin"
    echo "  - Container: /models/ggml/ggml-tiny.bin"
    echo ""
...
sed -i 's|WHISPER_MODEL_PATH: "../models/.cache/ggml/ggml-tiny.bin"|WHISPER_MODEL_PATH: "/models/ggml/ggml-tiny.bin"|g' /tmp/orchestra-dataflow.yml
...
echo "  - Whisper Model: ${WHISPER_MODEL_PATH:-/models/ggml/ggml-tiny.bin}"
echo ""
